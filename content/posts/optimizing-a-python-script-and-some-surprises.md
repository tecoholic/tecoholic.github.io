...
language to improve the performance? So, I launched Claude Code and asked it to rewrite
the Python code into Go lang, compile and benchmark after validating the outputs are the same.

It did a great job doing with the rewrite. The processing time went from `2 mins` on the `20MB` file to `~20 seconds`.
I was excited, but I noticed that the CPU was just maxing out at 100%, using just 1 core. Hmm... isn't Go known for its
concurrency. Why am I not seeing the benefits? I know in the back of my head, this is serial IO
processing. Concurrency isn't going to be of much use. Also, I don't know much about concurrency [^1]. But
...
1. The biggest improvements came from the file IO and Regex optimizations that, as per Claude, was O(n^2) and is now O(n)
2. I am spending time optimizing a Go program that I didn't write. So, I won't know if it's going in the right direction. [^2]

So, I asked Claude to analyze the Python script for performance bottlenecks. It came back with a couple of things. [^3]

* Compile multiple the single-line Regexes into a single regex.
* Using a custom function for replacement instead of using the `for regex in list_of_regexes` loop.  
* Implemente a custom character level processor to identify passages instead of a naive string matching that I had.

Once these were implemented, the runtime went from **2 mins** to **2.03 seconds**.

## Wait! What?

Yup. The optimized Python version was faster than the Go version. I just couldn't believe it.  Claude explained it with something like:

> Python's Regex engine implemented in C is way more performant than Go's version and Go is very conservative with Regexes.

Okay! Can we make the Go version faster with similar optimizations? Claude replaced some Regex's with string replacements and made it
...
I hear you. This post won't feel complete without Rust. So, I repeated the process with Rust. Initially, I
made the mistake of pointing Claude to the Go version as a reference for running the benchmarking
tests. So it implemented a character level version parsing and replacement. The code was just
too complex for me to understand. When I regenerated it from the optimized Python version, it was pretty comparable.
It also ended up being twice as fast.
...
This was a very fun excercise that taught me a few things. I am not going to be using the Go or the Rust version of the code due to project requirements and also the fact, I don't have the expertise to modify them later if needed. This sort of experimentation wouldn't have been possible a year back (maybe 2). It's amazing to see what LLMs can do. 
...
[^1]: I once tried to parallelize something using Python and launched about 10x the processes as the number of cores in my system. Everything ground to a halt and my CPU probably would have melted, if I hadn't pressed the power button.
[^2]: Running parallel workers for a serial task sounds sub-optimal, but I don't know enough about Go to make that call. I have often come across the phrase *zero-cost concurrency*. So, maybe Claude is right?
[^3]: I am deliberately being short on details here. Because, the optimization techniques themselves are not the message of this post.
